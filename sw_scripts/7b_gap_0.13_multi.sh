#!/bin/bash
#SBATCH --job-name=7b_gap_0.13_multi    
#SBATCH --output=logs/7b_gap_0.13_multi_%A.out
#SBATCH --error=logs/7b_gap_0.13_multi_%A.err
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
TMP_RUN_ROOT="${TMP_BASE%/}/7b_gap_0.13_multi_${USER}/${JOB_RUN_ID}"
...
RUN_NAME="${RUN_NAME:-7b_gap_0.13_multi_${RUN_ID}}"

LOG_DIR="${REBEL_LOG_DIR:-../logs}"
LOG_OUT="${LOG_DIR%/}/7b_gap_0.13_multi.out"
LOG_ERR="${LOG_DIR%/}/7b_gap_0.13_multi.err"
mkdir -p "$LOG_DIR"
...
HF_REPO_NAME="zjhhhh/7b_gap_0.13_multi"

############################
# Training configuration   #
############################
GRADIENT_ACCUMULATION_STEPS=$((128 / WORLD_SIZE))
TRAIN_INPUT_REPO="zjhhhh/7b_iter1_mean_beta_1.0_multi_expand_tokenized_gap_ratio_0.13"
