#!/bin/bash
#SBATCH --job-name=avgcheck
#SBATCH --output=logs/avgcheck_%A.out
#SBATCH --error=logs/avgcheck_%A.err
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
TMP_RUN_ROOT="${TMP_BASE%/}/avgcheck_${USER}/${JOB_RUN_ID}"
...
RUN_NAME="${RUN_NAME:-avgcheck_${RUN_ID}}"

LOG_DIR="${REBEL_LOG_DIR:-../logs}"
LOG_OUT="${LOG_DIR%/}/avgcheck.out"
LOG_ERR="${LOG_DIR%/}/avgcheck.err"
mkdir -p "$LOG_DIR"
...
HF_REPO_NAME="zjhhhh/avgcheck"

############################
# Training configuration   #
############################
GRADIENT_ACCUMULATION_STEPS=$((128 / WORLD_SIZE))
TRAIN_INPUT_REPO="zjhhhh/avgcheck_mean_beta_1.0_avgcheck_multi_expand_tokenized_gap_ratio_0.4"
