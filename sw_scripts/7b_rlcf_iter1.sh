#!/bin/bash
#SBATCH --job-name=7b_rlcf_iter1
#SBATCH --output=logs/7b_rlcf_iter1_%A.out
#SBATCH --error=logs/7b_rlcf_iter1_%A.err
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
TMP_RUN_ROOT="${TMP_BASE%/}/7b_rlcf_iter1_${USER}/${JOB_RUN_ID}"
...
RUN_NAME="${RUN_NAME:-7b_rlcf_iter1_${RUN_ID}}"

LOG_DIR="${REBEL_LOG_DIR:-../logs}"
LOG_OUT="${LOG_DIR%/}/7b_rlcf_iter1.out"
LOG_ERR="${LOG_DIR%/}/7b_rlcf_iter1.err"
mkdir -p "$LOG_DIR"
...
HF_REPO_NAME="zjhhhh/7b_rlcf_iter1"
