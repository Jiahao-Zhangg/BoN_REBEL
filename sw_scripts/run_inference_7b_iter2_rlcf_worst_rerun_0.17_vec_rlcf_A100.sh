#!/bin/bash
# SLURM script to run vector-RLCF inference across 24 shards using 3 jobs on A100
# Each job runs 8 Python scripts on different single GPUs (0,1,2,3,4,5,6,7)
# Shards 48-71 (24 total shards)
# Usage: sbatch sw_scripts/run_inference_7b_iter2_rlcf_worst_rerun_0.17_vec_rlcf_A100.sh
#SBATCH --job-name=inference_7b_iter2_rlcf_worst_vec_A100
#SBATCH --output=logs/inference_7b_iter2_rlcf_worst_vec_A100_%A_%a.out
#SBATCH --error=logs/inference_7b_iter2_rlcf_worst_vec_A100_%A_%a.err
#SBATCH --array=0-2
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
#SBATCH --exclusive
#SBATCH --nodelist=ip-10-1-173-179,ip-10-1-184-205,ip-10-1-196-96
...
# Available nodes for assignment (one job per node)
# Job 0 -> Node 0, Job 1 -> Node 1, Job 2 -> Node 2
AVAILABLE_NODES=("ip-10-1-173-179" "ip-10-1-184-205" "ip-10-1-196-96")
...
# Define paths
SCRIPT_PATH="src/checklist_judge_data_parallel/run_inference_on_shard_iter2_robust_vec_rlcf.py"
SHARD_DIR="./7b_iter2_rlcf_worst_rerun_0.17_vec_rlcf_shards"
OUTPUT_DIR="./inference_scores_7b_iter2_rlcf_worst_rerun_0.17_vec_rlcf"
JUDGE_MODEL="Qwen/Qwen3-14B"

# Calculate shard indices for this job (8 shards per job, starting from shard 48)
BASE_SHARD=$((48 + SLURM_ARRAY_TASK_ID * 8))
...
# Function to run inference on a specific GPU with error isolation
run_inference() {
    local shard_idx=$1
    local gpu=$2
    local gpu_id=$3

    # Skip if shard index exceeds 71 (we process shards 48-71)
    if [ "$shard_idx" -gt 71 ]; then
        echo "Skipping shard $shard_idx (exceeds maximum shard index 71)"
        return 0
    fi
    ...
    local shard_log_out="logs/rlcf_worst_vec_shard_${shard_idx_padded}_gpu${gpu_id}_${SLURM_JOB_ID}.out"
    local shard_log_err="logs/rlcf_worst_vec_shard_${shard_idx_padded}_gpu${gpu_id}_${SLURM_JOB_ID}.err"
    ...
    (
        ...
        python "$SCRIPT_PATH" \
            ...
            --adversary_pairs 2 \
            --push_to_hub \
            --hf_repo_template 'zjhhhh/7b_iter2_rlcf_worst_rerun_0.17_vec_rlcf_scores_{shard_idx}' \
