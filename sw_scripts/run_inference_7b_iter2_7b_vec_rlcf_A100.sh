#!/bin/bash
# SLURM script to run fullcheck RLCF (pointwise baseline) inference across 24 shards using 3 jobs on A100
# Each job runs 8 Python scripts on different single GPUs (0,1,2,3,4,5,6,7)
# Shards 48-71 (24 total shards)
#SBATCH --job-name=inference_7b_iter2_7b_vec_rlcf_A100
#SBATCH --output=logs/inference_7b_iter2_7b_vec_rlcf_A100_%A_%a.out
#SBATCH --error=logs/inference_7b_iter2_7b_vec_rlcf_A100_%A_%a.err
#SBATCH --array=0-2
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
SHARD_DIR="./7b_iter2_vec_rlcf_shards"
OUTPUT_DIR="./inference_scores_7b_iter2_vec_rlcf"
...
run_inference() {
    ...
    local shard_log_out="logs/vec_rlcf_shard_${shard_idx_padded}_gpu${gpu_id}_${SLURM_JOB_ID}.out"
    local shard_log_err="logs/vec_rlcf_shard_${shard_idx_padded}_gpu${gpu_id}_${SLURM_JOB_ID}.err"
