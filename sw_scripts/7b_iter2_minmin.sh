...
#SBATCH --output=logs/7b_iter2_minmin_%A.out
#SBATCH --error=logs/7b_iter2_minmin_%A.err
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
TMP_RUN_ROOT="${TMP_BASE%/}/7b_iter2_minmin_${USER}/${JOB_RUN_ID}"
...
RUN_NAME="${RUN_NAME:-7b_iter2_minmin_${RUN_ID}}"

LOG_DIR="${REBEL_LOG_DIR:-../logs}"
LOG_OUT="${LOG_DIR%/}/7b_iter2_minmin.out"
LOG_ERR="${LOG_DIR%/}/7b_iter2_minmin.err"
mkdir -p "$LOG_DIR"
...
TRAIN_INPUT_REPO="zjhhhh/iter2_7b_min_0.17_adversary_min_expand_ver2_tokenized_gap_ratio_0.17"
