#!/bin/bash

#SBATCH --job-name=qwen2.5_rlcf_game_value
#SBATCH --output=logs/qwen2.5_rlcf_game_value_%j.out
#SBATCH --error=logs/qwen2.5_rlcf_game_value_%j.err
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=24
#SBATCH --mem=256G
#SBATCH --partition=ml.p5.48xlarge
#SBATCH --nodelist=ip-10-1-81-8
#SBATCH --exclusive

set -euo pipefail

mkdir -p logs

echo "Python path: $(which python)"
echo "Conda environment: ${CONDA_DEFAULT_ENV:-not activated}"

# Match the cluster layout used by the existing game-value launchers while
# allowing the repository location to be overridden when submitting the job.
REPO_DIR="${REPO_DIR:-/fsx/gstevenw/testing_alignment_algos/BoN_REBEL}"
cd "$REPO_DIR"
echo "Switched to $(pwd)"

if [[ -f "/fsx/gstevenw/miniconda3/etc/profile.d/conda.sh" ]]; then
  source /fsx/gstevenw/miniconda3/etc/profile.d/conda.sh
elif [[ -f "${HOME}/miniconda3/etc/profile.d/conda.sh" ]]; then
  source "${HOME}/miniconda3/etc/profile.d/conda.sh"
fi
conda activate new_vllm
echo "Activated conda env: $(conda info --envs | awk '/\*/ {print $1}')"

export CUDA_DEVICE_ORDER=PCI_BUS_ID
export TOKENIZERS_PARALLELISM=false

# Keep the evaluation configuration aligned with game_value_7b_model.sh.
DATASET_REPO="zjhhhh/stage2_preprocessed"
BASE_MODEL="Qwen/Qwen2.5-7B-Instruct"
CHECKPOINT_MODEL="viswavi/qwen2.5_rlcf"
JUDGE_MODEL="Qwen/Qwen3-14B"

# Use an independent namespace so this run cannot overwrite the multi-model
# game_7b artifacts.
OUTPUT_REPO_PREFIX="zjhhhh/game_qwen2.5_rlcf"
BASE_OUTPUT_REPO="${OUTPUT_REPO_PREFIX}_base"

N_RESPONSE=8
N_JUDGE_SAMPLES=5
SAMPLE_SIZE=500

bash sw_scripts/game_value_parallel.sh \
  --dataset_repo "$DATASET_REPO" \
  --base_output_repo "$BASE_OUTPUT_REPO" \
  --base_model "$BASE_MODEL" \
  --check_points "$CHECKPOINT_MODEL" \
  --output_repo_prefix "$OUTPUT_REPO_PREFIX" \
  --judge_model "$JUDGE_MODEL" \
  --gpu_groups "0" \
  --extra_args_base "--n_response $N_RESPONSE --world_size 2 --base_gpus 0,1 --temperature 0.8 --end_idx $SAMPLE_SIZE" \
  --extra_args_eval "--world_size 1 --n_response $N_RESPONSE --n_judge_samples $N_JUDGE_SAMPLES --model_temperature 0.1 --switch_position" \
  --extra_args_score "--beta 1.0"

echo "Job finished at $(date)"
