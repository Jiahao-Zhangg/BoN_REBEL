#!/bin/bash
#SBATCH --job-name=7b_iter2_multi
#SBATCH --output=logs/7b_iter2_multi_%A.out
#SBATCH --error=logs/7b_iter2_multi_%A.err
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
TMP_RUN_ROOT="${TMP_BASE%/}/7b_iter2_multi_${USER}/${JOB_RUN_ID}"
...
RUN_NAME="${RUN_NAME:-7b_iter2_multi_${RUN_ID}}"

LOG_DIR="${REBEL_LOG_DIR:-../logs}"
LOG_OUT="${LOG_DIR%/}/7b_iter2_multi.out"
LOG_ERR="${LOG_DIR%/}/7b_iter2_multi.err"
mkdir -p "$LOG_DIR"
...
TRAIN_INPUT_REPO="zjhhhh/iter2_7b_multi_gap_0.15_scores_mean_beta_1.0_multi_0.15_tokenized"
