#!/bin/bash
# SLURM script to run min perprompt robust inference across 24 shards using 3 jobs on A100
# Each job runs 8 Python scripts on different single GPUs (0,1,2,3,4,5,6,7)
# Shards 48-71 (24 total shards)
# Usage: sbatch run_inference_iter2_7b_min_perprompt_A100.sh
#SBATCH --job-name=inference_iter2_7b_min_perprompt_A100
#SBATCH --output=logs/inference_iter2_7b_min_perprompt_A100_%A_%a.out
#SBATCH --error=logs/inference_iter2_7b_min_perprompt_A100_%A_%a.err
#SBATCH --array=0-2
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
SHARD_DIR="./iter2_7b_min_perprompt_shards"
OUTPUT_DIR="./inference_scores_iter2_7b_min_perprompt"
...
run_inference() {
    ...
    (
        ...
        python $SCRIPT_PATH \
            ...
            --switch_position \
            --push_to_hub \
            --hf_repo_template zjhhhh/iter2_7b_min_perprompt_scores_{target}_{shard_idx} \
