#!/bin/bash
#SBATCH --job-name=7b_sweep_eta_1e2
#SBATCH --output=logs/7b_sweep_eta_1e2_%A.out
#SBATCH --error=logs/7b_sweep_eta_1e2_%A.err
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
TMP_RUN_ROOT="${TMP_BASE%/}/7b_sweep_eta_${ETA}_${USER}/${JOB_RUN_ID}"
...
RUN_NAME="${RUN_NAME:-7b_sweep_eta_${ETA}_${RUN_ID}}"

LOG_DIR="${REBEL_LOG_DIR:-../logs}"
LOG_OUT="${LOG_DIR%/}/7b_sweep_eta_${ETA}.out"
LOG_ERR="${LOG_DIR%/}/7b_sweep_eta_${ETA}.err"
mkdir -p "$LOG_DIR"
...
HF_REPO_NAME="zjhhhh/7b_sweep_eta_${ETA}"
