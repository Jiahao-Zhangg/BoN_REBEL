#!/bin/bash
#SBATCH --job-name=3b_rerun_rlcf_iter2
#SBATCH --output=logs/3b_rerun_rlcf_iter2_%A.out
#SBATCH --error=logs/3b_rerun_rlcf_iter2_%A.err
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
BETA="1"
TOTAL_EPISODES="50800"         # Episodes fed to rebel.py --total_episodes
...
TMP_RUN_ROOT="${TMP_BASE%/}/3b_rerun_rlcf_iter2_${USER}/${JOB_RUN_ID}"
...
RUN_NAME="${RUN_NAME:-3b_rerun_rlcf_iter2_${RUN_ID}}"

LOG_DIR="${REBEL_LOG_DIR:-../logs}"
LOG_OUT="${LOG_DIR%/}/3b_rerun_rlcf_iter2.out"
LOG_ERR="${LOG_DIR%/}/3b_rerun_rlcf_iter2.err"
mkdir -p "$LOG_DIR"
...
HF_REPO_NAME="zjhhhh/3b_rerun_rlcf_iter2"

############################
# Training configuration   #
############################
GRADIENT_ACCUMULATION_STEPS=$((128 / WORLD_SIZE))
TRAIN_INPUT_REPO="zjhhhh/3b_iter2_rlcf_scores_rlcf_expand_tokenized_gap_ratio_0.22"
# Base model used to initialize training
BASE_MODEL="zjhhhh/3b_rerun_rlcf_expand_eta_1e4_step_373_final"
