#!/bin/bash
#SBATCH --job-name=7b_gap_0.17
#SBATCH --output=logs/7b_gap_0.17_%A.out
#SBATCH --error=logs/7b_gap_0.17_%A.err
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
TMP_RUN_ROOT="${TMP_BASE%/}/7b_gap_0.17_${USER}/${JOB_RUN_ID}"
...
RUN_NAME="${RUN_NAME:-7b_gap_0.17_${RUN_ID}}"

LOG_DIR="${REBEL_LOG_DIR:-../logs}"
LOG_OUT="${LOG_DIR%/}/7b_gap_0.17.out"
LOG_ERR="${LOG_DIR%/}/7b_gap_0.17.err"
mkdir -p "$LOG_DIR"
...
HF_REPO_NAME="zjhhhh/7b_gap_0.17"

############################
# Training configuration   #
############################
GRADIENT_ACCUMULATION_STEPS=$((128 / WORLD_SIZE))
TRAIN_INPUT_REPO="zjhhhh/7b_iter1_min_expand_ver2_tokenized_gap_ratio_0.17"
