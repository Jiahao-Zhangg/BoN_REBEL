#!/bin/bash
#SBATCH --job-name=7b_iter2_minmin_final_1e4    
#SBATCH --output=logs/7b_iter2_minmin_final_1e4_%A.out
#SBATCH --error=logs/7b_iter2_minmin_final_1e4_%A.err
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
TMP_RUN_ROOT="${TMP_BASE%/}/7b_iter2_minmin_final_eta_${ETA}_${USER}/${JOB_RUN_ID}"
...
RUN_NAME="${RUN_NAME:-7b_iter2_minmin_final_eta_${ETA}_${RUN_ID}}"

LOG_DIR="${REBEL_LOG_DIR:-../logs}"
LOG_OUT="${LOG_DIR%/}/7b_iter2_minmin_final_eta_${ETA}.out"
LOG_ERR="${LOG_DIR%/}/7b_iter2_minmin_final_eta_${ETA}.err"
mkdir -p "$LOG_DIR"
...
HF_REPO_NAME="zjhhhh/7b_iter2_minmin_final_eta_${ETA}"

############################
# Training configuration   #
############################
GRADIENT_ACCUMULATION_STEPS=$((128 / WORLD_SIZE))
TRAIN_INPUT_REPO="zjhhhh/iter2_7b_min_0.17_final_adversary_min_expand_ver2_tokenized_gap_ratio_0.17"
# Base model used to initialize training
BASE_MODEL="zjhhhh/7b_gap_0.17_step_350_final"
