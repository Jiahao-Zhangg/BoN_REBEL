#!/bin/bash
#SBATCH --job-name=7b_min_perprompt_iter2
#SBATCH --output=logs/7b_min_perprompt_iter2_%A.out
#SBATCH --error=logs/7b_min_perprompt_iter2_%A.err
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
BETA="1"
TOTAL_EPISODES="39000"         # Episodes fed to rebel.py --total_episodes
...
TMP_RUN_ROOT="${TMP_BASE%/}/7b_min_perprompt_iter2_eta_${ETA}_${USER}/${JOB_RUN_ID}"
...
RUN_NAME="${RUN_NAME:-7b_min_perprompt_iter2_eta_${ETA}_${RUN_ID}}"

LOG_DIR="${REBEL_LOG_DIR:-../logs}"
LOG_OUT="${LOG_DIR%/}/7b_min_perprompt_iter2_eta_${ETA}.out"
LOG_ERR="${LOG_DIR%/}/7b_min_perprompt_iter2_eta_${ETA}.err"
mkdir -p "$LOG_DIR"
...
HF_REPO_NAME="zjhhhh/7b_min_perprompt_iter2_eta_${ETA}"

############################
# Training configuration   #
############################
GRADIENT_ACCUMULATION_STEPS=$((128 / WORLD_SIZE))
TRAIN_INPUT_REPO="zjhhhh/7b_min_perprompt_iter2_min_noexpand_tokenized"
# Base model used to initialize training
BASE_MODEL="zjhhhh/7b_min_perprompt_iter1_eta_1e3_step_332_final"
