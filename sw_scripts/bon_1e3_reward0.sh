#!/bin/bash
#SBATCH --job-name=bon_1e3_reward0
#SBATCH --output=logs/bon_1e3_reward0_%A.out
#SBATCH --error=logs/bon_1e3_reward0_%A.err
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
TMP_RUN_ROOT="${TMP_BASE%/}/bon_1e3_reward0_${USER}/${JOB_RUN_ID}"
...
RUN_NAME="${RUN_NAME:-bon_1e3_reward0_${RUN_ID}}"

LOG_DIR="${REBEL_LOG_DIR:-../logs}"
LOG_OUT="${LOG_DIR%/}/bon_1e3_reward0.out"
LOG_ERR="${LOG_DIR%/}/bon_1e3_reward0.err"
mkdir -p "$LOG_DIR"
...
HF_REPO_NAME="zjhhhh/bon_1e3_reward0"

############################
# Training configuration   #
############################
GRADIENT_ACCUMULATION_STEPS=$((128 / WORLD_SIZE))
TRAIN_INPUT_REPO="zjhhhh/Llama-3.2-3B-Instruct_multi_armo_2rewards_preprocessed_rewardidx0_tokenized_gap_0.2"
