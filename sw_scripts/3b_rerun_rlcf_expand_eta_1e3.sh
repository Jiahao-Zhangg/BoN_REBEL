#!/bin/bash
#SBATCH --job-name=3b_rerun_rlcf_expand_eta_1e3
#SBATCH --output=logs/3b_rerun_rlcf_expand_eta_1e3_%A.out
#SBATCH --error=logs/3b_rerun_rlcf_expand_eta_1e3_%A.err
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
BETA="1"
TOTAL_EPISODES="26300"         # Episodes fed to rebel.py --total_episodes
...
TMP_RUN_ROOT="${TMP_BASE%/}/3b_rerun_rlcf_expand_eta_1e3_${USER}/${JOB_RUN_ID}"
...
RUN_NAME="${RUN_NAME:-3b_rerun_rlcf_expand_eta_1e3_${RUN_ID}}"

LOG_DIR="${REBEL_LOG_DIR:-../logs}"
LOG_OUT="${LOG_DIR%/}/3b_rerun_rlcf_expand_eta_1e3.out"
LOG_ERR="${LOG_DIR%/}/3b_rerun_rlcf_expand_eta_1e3.err"
mkdir -p "$LOG_DIR"
...
HF_REPO_NAME="zjhhhh/3b_rerun_rlcf_expand_eta_1e3"

############################
# Training configuration   #
############################
GRADIENT_ACCUMULATION_STEPS=$((128 / WORLD_SIZE))
TRAIN_INPUT_REPO="zjhhhh/3b_iter1_rlcf_rlcf_expand_tokenized_gap_ratio_0.22"
