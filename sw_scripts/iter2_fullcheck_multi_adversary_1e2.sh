#!/bin/bash
#SBATCH --job-name=iter2_fullcheck_multi_adversary_1e2  
#SBATCH --output=logs/iter2_fullcheck_multi_adversary_1e2_%A.out
#SBATCH --error=logs/iter2_fullcheck_multi_adversary_1e2_%A.err
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
TMP_RUN_ROOT="${TMP_BASE%/}/iter2_fullcheck_multi_adversary_eta_${ETA}_${USER}/${JOB_RUN_ID}"
...
RUN_NAME="${RUN_NAME:-iter2_fullcheck_multi_adversary_eta_${ETA}_${RUN_ID}"

LOG_DIR="${REBEL_LOG_DIR:-../logs}"
LOG_OUT="${LOG_DIR%/}/iter2_fullcheck_multi_adversary_eta_${ETA}.out"
LOG_ERR="${LOG_DIR%/}/iter2_fullcheck_multi_adversary_eta_${ETA}.err"
mkdir -p "$LOG_DIR"
...
HF_REPO_NAME="zjhhhh/iter2_fullcheck_multi_adversary_eta_${ETA}"
