...
def load_judge_preferences(dataset_name, measure='majority'):
    ...
    try:
        ...
        for i, row in enumerate(data):
            ...
            if 'response_0_1_judged_preference_mean' in row and measure == 'mean':
                pref = row['response_0_1_judged_preference_mean']
                judge_preferences.append(pref)
                print(f"Row {i}: {pref}")
            elif 'response_0_1_judged_preference_majority' in row and measure == 'majority':
...
def compute_correlations(x_data, y_data, use_ternary_mapping=False):
    ...
    # Compute all three 
    print('L2 loss: ', np.mean((np.array(x_flat)-np.array(y_flat))**2))
...
def main():
    ...
    judge_data = load_judge_preferences(args.dataset, args.measure)
