...
def load_preference_5score_template() -> str:
    template_path = "prompt_preference_5score.txt"
...
def load_winrate_template() -> str:
    template_path = "prompt_winrate.txt"
    with open(template_path, 'r') as f:
        return f.read()


def build_winrate_prompts(template: str, unique_prompts: List[str], checks_by_prompt: Dict[str, List[str]], prompt_to_idx: Dict[str, int], base_resps: List[List[str]], model_resps: List[List[str]]) -> Tuple[List[List[dict]], List[Tuple[int, int, int]]]:
    prompts: List[List[dict]] = []
    pair_map: List[Tuple[int, int, int]] = []  # (prompt_idx, ai, bj)
    for p_idx, p in enumerate(unique_prompts):
        pid = prompt_to_idx[p]
        checks = checks_by_prompt.get(p, [])
        checks_str = "\n".join([f"- {c}" for c in checks]) if checks else "- Overall quality and helpfulness"
        for ai, a_text in enumerate(model_resps[pid]):
            for bj, b_text in enumerate(base_resps[pid]):
                instruction = template.format(prompt=p, response_a=a_text, response_b=b_text, checks=checks_str)
                prompts.append(get_message(instruction))
                pair_map.append((p_idx, ai, bj))
    return prompts, pair_map
...
async def compute_simple_winrate_async(client: AsyncOpenAI, template: str, judge_model: str, unique_prompts: List[str], checks_by_prompt: Dict[str, List[str]], prompt_to_idx: Dict[str, int], base_resps: List[List[str]], model_resps: List[List[str]], n_samples: int, max_concurrent: int) -> Tuple[List[int], List[int], List[float], float]:
    prompts, pair_map = build_winrate_prompts(template, unique_prompts, checks_by_prompt, prompt_to_idx, base_resps, model_resps)
    raw_lists = await batch_api_calls_async(client, prompts, judge_model, n_samples, max_concurrent)

    num_prompts = len(unique_prompts)
    wins_per_prompt: List[int] = [0] * num_prompts
    totals_per_prompt: List[int] = [0] * num_prompts

    for idx, samples in enumerate(raw_lists):
        p_idx, ai, bj = pair_map[idx]
        choice = aggregate_binary_choice(samples)
        if choice is None:
            continue
        totals_per_prompt[p_idx] += 1
        if choice == "A":  # model (A) vs base (B)
            wins_per_prompt[p_idx] += 1

    per_prompt_wr: List[float] = []
    for i in range(num_prompts):
        if totals_per_prompt[i] > 0:
            per_prompt_wr.append(wins_per_prompt[i] / totals_per_prompt[i])
        else:
            per_prompt_wr.append(None)

    valid = [v for v in per_prompt_wr if v is not None]
    avg_wr = (sum(valid) / len(valid)) if valid else None
    return wins_per_prompt, totals_per_prompt, per_prompt_wr, avg_wr
...
async def main():
    ...
    winrate_template = load_winrate_template()
    ...
    for model_id in args.check_points:
        ...
        # Compute simple winrate per prompt using all checks combined (no extra sampling of responses)
        checks_by_prompt: Dict[str, List[str]] = {}
        for row in expanded_rows:
            p = row["prompt"]
            c = row.get("check", p)
            checks_by_prompt.setdefault(p, []).append(c)
        wins, totals, per_prompt_wr, avg_wr = await compute_simple_winrate_async(
            client,
            winrate_template,
            args.judge_model,
            unique_prompts,
            ...
            base_resps,
            model_resps,
            args.n_judge_samples,
            args.max_concurrent,
        )
        # Attach to merged dataset and push
        ds_merged = ds_merged.add_column("win_count", wins)
        ds_merged = ds_merged.add_column("pair_count", totals)
        ds_merged = ds_merged.add_column("winrate", per_prompt_wr)
