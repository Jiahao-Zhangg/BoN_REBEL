...
RAW_REPO=${RAW_REPO:-"zjhhhh/iter2_fullcheck_multi_scores_base_rescaled"}
...
BASE_OUTPUT_PREFIX=${BASE_OUTPUT_PREFIX:-"zjhhhh/iter2_fullcheck_multi_base"}

# Derived repos
BASE_PREPROCESSED_REPO=${BASE_PREPROCESSED_REPO:-"${BASE_OUTPUT_PREFIX}_preprocessed"}
...
echo "[1/2] Preprocessing common dataset (iter2, intersection test) -> $BASE_PREPROCESSED_REPO"
python preprocess_common_stage2.py \
  --model "$MODEL" \
  --input_repo "$RAW_REPO" \
  --output_repo "$BASE_PREPROCESSED_REPO" \
  --maxlen "$MAXLEN" \
...
#   --gap_ratio "$GAP_RATIO" \
#   ${GAP_SHUFFLE_SEED:+--gap_shuffle_seed "$GAP_SHUFFLE_SEED"}
echo "[2/2] Running multi_expand filter for base -> prefix $BASE_OUTPUT_PREFIX (gap_ratio=$GAP_RATIO)"
python filter_tokenize_judge_fullcheck_expand.py \
  --model "$MODEL" \
