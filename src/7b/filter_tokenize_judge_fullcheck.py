...
def main():
    ...
    # Process a single split (no-expand: one pair per example)
    def process_split(dataset, is_train):
        ...
        # For the no-expand variant, we yield at most one (chosen, reject) pair per original row.
        def row_generator():
            for row in tqdm(dataset):
                ...
                # No-expand: pick the single best vs worst selection for this row.
                if not g_values:
                    continue
                g_array = np.array(g_values, dtype=float)
                if g_array.size < 2:
                    continue
                reject_idx = int(np.argmin(g_array))
                chosen_idx = int(np.argmax(g_array))
                ...
                example.update({
                    "chosen": row[f"selection_response_{chosen_sel_id}"],
                    "reject": row[f"selection_response_{reject_sel_id}"],
                    "qwen_chosen_tokens": selection_tokens[chosen_idx],
                    "qwen_reject_tokens": selection_tokens[reject_idx],
                    "qwen_chosen": selection_texts[chosen_idx],
                    "qwen_reject": selection_texts[reject_idx],
    ...
    base_repo = repo_prefix + '_beta_' + str(args.beta) + '_fullcheck_noexpand_tokenized'
