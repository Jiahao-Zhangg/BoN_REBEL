...
# RAW_REPO=${RAW_REPO:-"zjhhhh/7b_iter1"}
...
OUTPUT_PREFIX=${OUTPUT_PREFIX:-"zjhhhh/7b_min_iter1"}

# Derived repos
PREPROCESSED_REPO=${PREPROCESSED_REPO:-"zjhhhh/7b_iter1_preprocessed"}
...
#   --gap_ratio "$GAP_RATIO" \
#   ${GAP_SHUFFLE_SEED:+--gap_shuffle_seed "$GAP_SHUFFLE_SEED"}
echo "[2/2] Running min filter for 7b -> prefix $OUTPUT_PREFIX (gap_ratio=$GAP_RATIO)"
python filter_tokenize_judge_min.py \
  --model "$MODEL" \
  --input_repo "$PREPROCESSED_REPO" \
  --maxlen "$MAXLEN" \
  --maxlen_prompt "$MAXLEN_PROMPT" \
