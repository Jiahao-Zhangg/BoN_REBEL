...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description="Filter + tokenize using a fixed check while pooling current and selection responses.",
    )
...
def main():
    ...
    for row in tqdm(dataset):
        ...
        A_fixed = float(np.sum(list(exp_terms.values())))
        ...
        for cur_id in current_ids:
            ...
            g_val = float(weighted_sum / A_fixed)
            candidates.append({
                "g": g_val,
    ...
    dataset.push_to_hub(args.input_repo + '_' + args.score_type + '_maxlenp_' + str(args.maxlen_prompt)+'_beta_'+str(args.beta) + '_fixed_equal_tokenized')
