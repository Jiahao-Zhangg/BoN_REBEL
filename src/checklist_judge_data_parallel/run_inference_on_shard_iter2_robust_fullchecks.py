...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run judge inference on a prepared shard and compute pairwise scores between selection/current and base/adversary (mean and majority per pair).")
...
def main():
    ...
    # 3) selection vs adversary
    for i in range(1, args.selection_pairs + 1):
        sel_col = selection_cols[i - 1]
        for t in range(1, args.adversary_pairs + 1):
            label = f"selection_{i}_adversary_{t}_score"
            if label + "_mean" not in pair_results:
                adv_col = adversary_cols[t - 1]
                run_pair_and_reduce(sel_lists[sel_col], adv_lists[adv_col], label)
    # 4) current vs adversary
    for k in range(1, args.current_pairs + 1):
        cur_col = current_cols[k - 1]
        for t in range(1, args.adversary_pairs + 1):
            label = f"current_{k}_adversary_{t}_score"
            if label + "_mean" not in pair_results:
                adv_col = adversary_cols[t - 1]
                run_pair_and_reduce(cur_lists[cur_col], adv_lists[adv_col], label)
    ...
    # Write dataset for adversary pairs (selection/current vs adversary)
    adv_out_path = os.path.join(
        args.output_dir,
        f"shard_{args.idx:05d}_scores_adversary.jsonl",
    )
    adv_rows = []
    tmp_adv_path = adv_out_path + ".tmp"
    with open(tmp_adv_path, "w") as f:
        for oi in range(n_rows):
            row_out = {k: ds[oi][k] for k in ds.column_names}
            # selection vs adversary (scalar values per pair)
            for i in range(1, args.selection_pairs + 1):
                for t in range(1, args.adversary_pairs + 1):
                    key = f"selection_{i}_adversary_{t}_score"
                    mean_vals = pair_results.get(key + "_mean", [])
                    maj_vals = pair_results.get(key + "_majority", [])
                    mean_val = mean_vals[oi] if oi < len(mean_vals) else None
                    maj_val = maj_vals[oi] if oi < len(maj_vals) else None
                    row_out[key.replace("_score", "_mean")] = mean_val
                    row_out[key.replace("_score", "_majority")] = maj_val
            # current vs adversary (scalar values per pair)
            for k in range(1, args.current_pairs + 1):
                for t in range(1, args.adversary_pairs + 1):
                    key = f"current_{k}_adversary_{t}_score"
                    mean_vals = pair_results.get(key + "_mean", [])
                    maj_vals = pair_results.get(key + "_majority", [])
                    mean_val = mean_vals[oi] if oi < len(mean_vals) else None
                    maj_val = maj_vals[oi] if oi < len(maj_vals) else None
            ...
            f.write(json.dumps(row_out) + "\n")
            adv_rows.append(row_out)

    os.replace(tmp_adv_path, adv_out_path)
    print(f"Wrote {n_rows} rows -> {adv_out_path}")

    if args.push_to_hub:
        try:
            repo_id = resolve_repo_id("adversary")
            print(f"Pushing adversary dataset to HF Hub -> {repo_id}")
            ds_out = Dataset.from_list(adv_rows)
            ds_out.push_to_hub(repo_id)
            print(f"Pushed dataset to hub: {repo_id}")
        except Exception as e:
            print(f"Failed to push adversary dataset to Hugging Face Hub: {e}")
