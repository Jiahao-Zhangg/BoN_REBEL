...
def parse_arguments():
    ...
    parser.add_argument("--model", type=str, default="meta-llama/Llama-3.2-3B-Instruct") #used to be meta-llama/Meta-Llama-3-8B-Instruct
...
def main():
    ...
    for row in dataset:
        ...
        # Calculate weighted average rewards: 0.5 * reward_0_response_i + 0.5 * reward_1_response_i
        all_rewards = []
        for i in range(args.pairs):
            reward_0 = row[f"reward_0_response_{i}"]
            reward_1 = row[f"reward_1_response_{i}"]
            weighted_reward = 0.5 * reward_0 + 0.5 * reward_1
        ...
        # Use weighted average reward for chosen response
        chosen_weighted_reward = 0.5 * row[f"reward_0_response_{chosen_idx}"] + 0.5 * row[f"reward_1_response_{chosen_idx}"]
        chosen_reward.append(chosen_weighted_reward)
        ...
        # Use weighted average reward for reject response
        reject_weighted_reward = 0.5 * row[f"reward_0_response_{reject_idx}"] + 0.5 * row[f"reward_1_response_{reject_idx}"]
        reject_reward.append(reject_weighted_reward)
    ...
    dataset.push_to_hub(args.input_repo + '_two_header_tokenized')
