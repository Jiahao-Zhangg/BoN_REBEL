...
JOB_RUN_ID="${SLURM_JOB_ID:-$(date +%s)}"
TMP_RUN_ROOT="${TMPDIR:-/tmp}/nocheck_rebel_gap_${USER}/${JOB_RUN_ID}"
OUTPUT_DIR="${TMP_RUN_ROOT}/outputs_seed_${SEED}_beta_${BETA}_eta_${ETA}"
HF_REPO_NAME="zjhhhh/qwen2.5_3B_Instruct_nocheck_gap_seed_${SEED}_beta_${BETA}_eta_${ETA}"
EVAL_RESULTS_DIR="/project/flame/$USER/qwen2.5_3B_Instruct_nocheck_gap_evaluation_results"

# Derived configuration
GRADIENT_ACCUMULATION_STEPS=$((128 / WORLD_SIZE))

TRAIN_INPUT_REPO="zjhhhh/choose_gap_beta_1_nocheck_tokenized"

# Evaluation configuration
BASE_MODEL="Qwen/Qwen2.5-3B-Instruct"
EVAL_DATASET_NAME="zjhhhh/choose_gap_beta_1_nocheck_tokenized"
...
mkdir -p "$EVAL_RESULTS_DIR"
EVAL_RESULTS_FILE="$EVAL_RESULTS_DIR/reward_evaluation_seed_${SEED}_beta_${BETA}_eta_${ETA}_$(date +%Y%m%d_%H%M%S).txt"
